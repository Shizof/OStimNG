...
namespace HiggsPluginAPI
{
	...
	struct IHiggsInterface001
	{
        ...
        virtual RE::bhkRigidBody* GetHandRigidBody(bool isLeft) = 0;
        virtual RE::bhkRigidBody* GetWeaponRigidBody(bool isLeft) = 0;

        // Get the currently held rigid body. Return type is actually bhkRigidBody.
        virtual RE::bhkRigidBody* GetGrabbedRigidBody(bool isLeft) = 0;
        ...
        // Get the name of the node that the given hand is currently grabbing, or an empty string if not grabbing a node
        // or if the grabbed node has no name.
